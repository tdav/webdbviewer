...
- **Авто-alias при выборе таблицы** — `SemanticCompleter` (серверная часть, включается через `CompletionOptions.AutoAliasTables`). Клиентский JS-порт `makeAlias` существовал кратко в ходе этой итерации и был удалён коммитом `bb2699a`: он расходился с сервером (`AutoAliasTables` по умолчанию `false`, `/api/completion` его не включает) — один и тот же вариант вставлял разный текст до и после ответа сервера. Сейчас алиас на клиенте не добавляется.
...
- **Клиентский модуль снапшота** — `frontend/src/completion-schema.js`, экспортируется как `window.WebDbCompletion` (`load`, `reset`, `localCompletions`, `stats`). Хранит снапшот в `Map` по ключу `dsId/schema`, дедуплицирует параллельные загрузки (`inflight`), копирует серверное правило квотирования идентификаторов (`quote` — порт `SqlIdentifierQuoting.Quote`). Ключевые слова в локальные варианты подмешивает не этот модуль, а `editor.js` — встроенным `keywordCompletionSource` из `@codemirror/lang-sql`, свой список не заводится.
- **Двухфазный источник автодополнения** — `frontend/src/editor.js`, `makeCompletionSource`: фаза 1 отдаёт локальные варианты из снапшота мгновенно (без сети), параллельно с текущим debounce 250 мс уходит запрос на сервер; фаза 2 — когда серверный ответ приходит (сверка по ключу «датасорс + схема + позиция + текст»), вызывается `startCompletion(view)` и на повторном проходе источник возвращает объединённый список (серверные варианты первыми, локальные добавляются только те, чьих `label` там нет). Прежний вызов прогрева заменён на `loadSchemaMap` (три точки вызова: инициализация редактора, смена схемы, смена датасорса).
...
- **Замеры латентности** — сервер: `src/WebDbViewer.Metadata/MetadataCache.cs`, лог уровня `Debug` в `LoadCoreAsync` — `"Интроспекция схемы {Schema} датасорса {DataSourceId}: {ElapsedMs} мс, таблиц {Tables}"`; аналогичный лог уровня `Debug` в `CompletionEndpoints.CompleteAsync` — длительность обработки `/api/completion`. Клиент: `performance.now()` вокруг `localCompletions`, накопление в `window.WebDbCompletion.stats()` (`{count, p50, p95}`); вывод в консоль раз в 5 с — только при `?debugCompletion=1` в URL (`frontend/src/editor.js`).
