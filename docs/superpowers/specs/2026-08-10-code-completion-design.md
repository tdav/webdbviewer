...
Шаблонные сниппеты запросов (`SELECT … FROM … WHERE`) не делаются: в редакторе с рабочим
автодополнением они экономят меньше, чем занимают места в списке.

## Второй заход

Выполнен следом по решению заказчика; исходно эти пункты были вынесены за рамки.

**Справочник диалекта** (`DialectVocabulary`). Каталог БД не отдаёт ни `SYSDATE`, ни `coalesce`,
поэтому встроенные функции, константы, псевдоколонки и типы данных заданы статически, отдельно
для Oracle и PostgreSQL. Встроенные функции идут ниже пользовательских: те специфичны для схемы.

**Типы данных** предлагаются там, где тип — единственное осмысленное продолжение: после `::`
в PostgreSQL и внутри `CAST(… AS |)`. Одного слова `AS` мало — вне `CAST` это алиас, поэтому
проверяется охватывающий вызов.

**Oracle-специфика.** `DUAL` принадлежит `SYS` и в снапшот схемы пользователя не попадает, хотя
во `FROM` встречается постоянно. Неопознанный квалификатор (`order_seq.`) даёт `NEXTVAL`/`CURRVAL`:
последовательностей в снапшоте нет, а пустой список хуже двух верных вариантов. Подпрограммы
`DBMS_*` живут в `SYS` и добавлены в справочник полными именами.

**`VALUES`** проваливался в v0-эвристику и предлагал колонки, которые там ни при чём. Теперь —
значения, функции, `DEFAULT` и `NULL`. В `SET` к работавшим колонкам добавлены функции.

**История использования** (`RecentObjects`). LRU на 20 таблиц per datasource, пополняется из
текста при каждом запросе автодополнения; таблица из истории поднимается на один разряд
приоритета. Живёт в памяти процесса — переживать перезапуск истории подсказок не обязана.

**Signature help.** `POST /api/completion/signature` разбирает вызов под кареткой
(`CaretText.EnclosingCall`) и ищет сигнатуру сначала среди `Routines` кэша, затем в справочнике
диалекта; 204 — вызова нет или он неизвестен. В редакторе — `StateField` + `showTooltip`,
подсказка держится, пока каретка внутри скобок. Ключевые слова (`SELECT`, `IN`, `EXISTS`, `OVER`)
исключены из распознавания вызова, иначе группирующая скобка порождала бы запрос.
