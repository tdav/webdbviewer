...
Экспорт и импорт открывают **собственное соединение** через `IDbConnectionFactory`,
а не занимают сессию пользователя. Причина: обе операции длятся минуты, а сессия — это
одно физическое соединение, которое всё это время было бы недоступно для запросов.
Побочная выгода у импорта — он не втягивается в открытую транзакцию пользователя
и не может её откатить. Плата у экспорта — скрипт не видит незакоммиченных правок
той транзакции. Тот же приём применён в проекте к чтению DDL и списков схем.

### UI

- `Pages/Shared/_TreeNode.cshtml` — кнопка экспорта у узлов **Table** (не View: там
  «структура + данные» означало бы `CREATE VIEW` и выгрузку производных строк).
  Обычный клик открывает скрипт вкладкой редактора с пределом строк, Ctrl+клик уводит
  на `/api/export/sql` и скачивает таблицу целиком — тот же приём, что у ссылки на данные.
- Тулбар редактора — кнопка импорта, открывающая нативный `<dialog>` с выбором файла
  и одного из трёх действий: открыть в редакторе не выполняя (по умолчанию — пользователь
  видит, что запускает), выполнить в одной транзакции, выполнить продолжая после ошибок.
- Тулбар редактора — кнопка экспорта результата запроса: берёт SQL активной вкладки,
  спрашивает имя таблицы-приёмника и скачивает файл с INSERT-операторами.
...
Тесты самодостаточны: сами создают таблицу с разнотипными данными, поэтому не требуют
заранее подготовленной демо-схемы — достаточно любой базы, где есть право на CREATE TABLE.

### Состояние прогона на 2026-08-10

- Unit: 64 теста экспорта, все зелёные (в сумме по проекту — 363).
- Integration PostgreSQL: 4 round-trip теста прошли на живом сервере.
- Integration Oracle: 4 теста написаны, но **пропущены** — инстанса не было.
  Oracle-путь подтверждён только unit-тестами литералов; round-trip требует прогона
  с заданной `WEBDBVIEWER_TEST_ORACLE`.
