...
| Пункт | Состояние | Комментарий |
|---|---|---|
| `dotnet build` | Работает | 0 ошибок; предупреждение NU1903 по `SQLitePCLRaw` (остаточная SQLite-зависимость) |
| `dotnet test` (unit) | Работает | 219 тестов |
...
4. Снять остаточную SQLite-зависимость вместе с неиспользуемыми файловыми хранилищами (уязвимость NU1903).
